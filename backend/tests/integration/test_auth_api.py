...
All tests run through the full ASGI stack. The JSON DB is patched to a
temp file so tests never touch data/db.json on disk.
"""
import json
...
def db_file(tmp_path: Path) -> Path:
    """Minimal seeded db.json in a temp dir."""
    db = {
...
@pytest.fixture
def auth_client(api_client: TestClient, db_file: Path) -> TestClient:
    """TestClient with the store patched to use the temp db file."""
...
def test_refresh_issues_new_token_pair(auth_client: TestClient, db_file: Path):
...
def test_refresh_rejects_access_token(auth_client: TestClient, db_file: Path):
...
def test_refresh_rejects_revoked_token(auth_client: TestClient, db_file: Path):
...
def test_protected_route_accepts_valid_token(auth_client: TestClient, db_file: Path):
...
def test_middleware_rejects_refresh_token_on_protected_route(auth_client: TestClient, db_file: Path):
...
def test_middleware_rejects_revoked_access_token(auth_client: TestClient, db_file: Path):
